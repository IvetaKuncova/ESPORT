...
-- Http address does not support certain UTF-8 characters (symbols, none ASCII alphabets (Cyrillic, Japanese, Chinese) etc.), 
...
Unwanted characters are divided into 3 groups:
1. Characters to substitute for another one (e.g.: ě -> e) or 
2. Charaters to delete (e.g.: お).
   - For these two groups is the TRANSLATE function ideal.
...
SELECT 
    "PlayerId", 
    -- For correctness check, I added original columns (without replacing/deleting changes), but they are not necessary
    "CurrentHandle",
...
and  also names of players from Asia have the Last name on the first place, 
which was taken into account in the case of http addresses 
(Special Column "ShowLastNameFirst", which was in data from APIs 08 and 10).
...
SELECT 
    ...
    -- set of players' ids with Last-Name-First is much smaller than the opposite, therefore managing them first is quicker for processing
...
There were also players born in 2023 in the data, which doesn't really make sense, and it is misfilled data.) 

Dates of Birth wich were not filled (or 'unknown'), were excluded.
*/
