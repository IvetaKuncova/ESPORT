"""
12 LookupHighestEarningTeamsByGame
Script for esportsearning.com API: LookupHighestEarningTeamsByGame 
URL: http://api.esportsearnings.com/v0/LookupHighestEarningTeamsByGame?apikey=<apikey>&gameid=<gameid>&offset=<offset>
...
gameid = 151 # !!! ID You want to start on
...
file_name = f"12-API-LookupHighestEarningTeamsByGame_{gameid}-{gameid+numberofID-1}"
...
estim_time_of_execution = numberofID * stopoffset 
...
def get_LookupHighestEarningTeamsByGame(gameid, offset):
    numOfGame = 1
    games = []
    for i in range(0, stopoffset):
        # API requires min 1 second between queries, min 1 second between requests
        time.sleep(1.0)
        url = f'https://api.esportsearnings.com/v0/LookupHighestEarningTeamsByGame?apikey={apikey}&gameid={gameid}&offset={offset}'
        ...
        # if the API returns "NULL"/none:
            ...
            for dictionary in data:
                ...
                dictionary["numOfGame"] =  numOfGame
                numOfGame += 1
        games.append(data)
        offset += 100
    return games
...
try:
    with open(f"{file_name}.jsonl", mode="w", encoding="utf-8") as file: 
        for i in range(0,numberofID):
            ...
            dataA = get_LookupHighestEarningTeamsByGame(gameid + i, offset) 
            if not dataA:
                continue
            for listA in dataA:
    ...
    with open(f'{file_name}.csv', 'w', newline='', encoding="utf8") as fw:
        with open(f'{file_name}.jsonl', 'r', encoding="utf8") as fr:
            ...
            for row in fr:
                ...
                if first_row:
                    first_row = False
                    headers = list(data.keys()) + ["ErrorCode"]
