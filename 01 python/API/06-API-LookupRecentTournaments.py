"""
06 LookupRecentTournaments 
Script for esportsearning.com API: LookupRecentTournaments 
URL: http://api.esportsearnings.com/v0/LookupRecentTournaments?apikey=<apikey>&offset=<offset>
...
file_name = f"06-API-LookupRecentTournaments_{offset}-{(offset+stopoffset*100)-100}"
...
def get_LookupRecentTournaments(offset):
    # API requires min 1 second between queries, min 1 second between requests
    time.sleep(1.0)
    url = f'https://api.esportsearnings.com/v0/LookupRecentTournaments?apikey={apikey}&offset={offset}'
...
try:
    with open(f"{file_name}.jsonl", mode="w", encoding="utf-8") as file:
        for i in range(0, stopoffset):
            ...
            dataA = get_LookupRecentTournaments(offset)
            offset += 100 
            if not dataA:
                continue
            for listA in dataA:
