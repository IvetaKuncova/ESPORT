"""
02 LookupPlayerTournaments
Script for esportsearning.com API: LookupPlayerTournaments 
URL: http://api.esportsearnings.com/v0/LookupPlayerTournaments?apikey=<apikey>&playerid=<playerid>&offset=<offset>
...
stopoffset = 700 # !!! How many offsets should be processed?
file_name = f"02-API-LookupPlayerTournaments_{playerid}-{playerid+numberofID-1}"
...
def get_LookupPlayerTournaments(playerid, offset):
    numOfTournament = 1
    tournaments = []
    for i in range(0, stopoffset):
        # API requires min 1 second between queries, min 1 second between requests
        time.sleep(1.0)
        url = f'https://api.esportsearnings.com/v0/LookupPlayerTournaments?apikey={apikey}&playerid={playerid}&offset={offset}'
        ...
        # if the API returns "NULL"/none:
            ...
            try:
                ...
                for dictionary in data:
                    ...
                    dictionary["NumOfTournament"] = numOfTournament
                    numOfTournament += 1
            except:
...
try:
    with open(f"{file_name}.jsonl", mode="w", encoding="utf-8") as file: 
        for i in range(0,numberofID):
            ...
            dataA = get_LookupPlayerTournaments(playerid + i, offset) 
            if not dataA:
                continue
            for listA in dataA:
