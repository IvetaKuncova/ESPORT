"""
05 LookupHighestEarningPlayersByGame
Script for esportsearning.com API: LookupHighestEarningPlayersByGame
URL: http://api.esportsearnings.com/v0/LookupHighestEarningPlayersByGame?apikey=<apikey>&gameid=<gameid>>&offset=<offset>
...
gameid = 253 # !!! ID You want to start on
numberofID =  1 # !!! ID You want to start on
offset = 0 # !!! starting offset (how many tournaments should be skipped)
stopoffset = 1 # !!! How many offsets should be processed?
file_name = f"05-API-LookupHighestEarningPlayersByGame_{gameid}-{gameid+numberofID-1}"
...
estim_time_of_execution = numberofID * 1.6859
...
def get_LookupHighestEarningPlayersByGame(gameid, offset):
    numOfPlayer = 1
    players = []
    for i in range(0, stopoffset):
        # API requires min 1 second between queries, min 1 second between requests
        time.sleep(1.0)
        url = f'https://api.esportsearnings.com/v0/LookupHighestEarningPlayersByGame?apikey={apikey}&gameid={gameid}&offset={offset}'
        ...
        if response.status_code != 200: 
            ...
            errorCodes.append(f"GameId:{gameid}-error:{response.status_code}")  
        ...
        # if the API returns "NULL"/none:
            ...
            # Add a new keys and ID values
            for dictionary in data:
                dictionary["GameId"] = gameid
                dictionary["NumOfPlayer"] = numOfPlayer
                numOfPlayer += 1
        players.append(data)
        offset += 100
    return players
...
try:
    with open(f"{file_name}.jsonl", mode="w", encoding="utf-8") as file: 
        for i in range(0,numberofID):
            # Call function with parameter of firt ID, where you want start and offset
            dataA = get_LookupHighestEarningPlayersByGame(gameid + i, offset) 
            if not dataA:
                continue
            for listA in dataA:
                for dictionary in listA:
                    # save dictionary in to the "file"
...
def jsonl_to_csv():     
    with open(f'{file_name}.csv', 'w', newline='', encoding="utf8") as fw:
