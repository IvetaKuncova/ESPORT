"""
11 LookupHighestEarningTeams
Script for esportsearning.com API: LookupHighestEarningTeams
URL: http://api.esportsearnings.com/v0/LookupHighestEarningTeams?apikey=<apikey>&offset=<offset>
...
file_name = f"11-API-LookupHighestEarningTeams_{offset}-{offset+(stopoffset*100)-100}"
...
estim_time_of_execution = stopoffset * 0.5
...
def get_LookupHighestEarningTeams(offset):
    # API requires min 1 second between queries, min 1 second between requests
    time.sleep(1.0)
    url = f'https://api.esportsearnings.com/v0/LookupHighestEarningTeams?apikey={apikey}&offset={offset}'
...
try:
    with open(f"{file_name}.jsonl", mode="w", encoding="utf-8") as file:
        for i in range(0, stopoffset):
            ...
            dataA = get_LookupHighestEarningTeams(offset)
            offset += 100 
            if not dataA:
                continue
            for seznamA in dataA:
                json.dump(seznamA, file, ensure_ascii=False) 
                file.write("\n") # separate lines
